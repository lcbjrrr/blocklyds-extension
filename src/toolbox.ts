...
pythonGenerator['aggFunc'] = function(block: Blockly.Block): [string, number] {
  ...
  const code: string = `${dataframe}.${aggFunction}()[${columnCode}]`;
  return [code, pythonGenerator.ORDER_FUNCTION_CALL];
};
...
Blockly.Blocks['selector_train_test_split'] = {
   ...
   */
  init: function() {
    // Input for train/test split selection
...
Blockly.Blocks['linear_regression_predictor'] = {
   ...
   */
  init: function() {
    // Input for the linear regression model
    this.appendValueInput("model")
      .setCheck("Array")
      .appendField("(LR Predictor)")
      .appendField("Model:");
    
    // Input for features on which predictions are based
    this.appendValueInput("features")
      .setCheck("Array")
      .appendField("                    Features:");
...
pythonGenerator['confusionMatrix'] = function(block: Blockly.Block): [string, number] {
  ...
  pythonGenerator.addImport('from sklearn.metrics import confusion_matrix');
...
Blockly.Blocks['correlation'] = {
  init: function() {
    // Adiciona um campo de entrada de valor para o DataFrame
    this.appendValueInput('dataframe')
      .appendField('(Correlation)')  // Título do bloco: Correlation
      .appendField('DataFrame:');    // Texto ao lado do campo de entrada para o DataFrame

    // Define que este bloco terá uma saída
    this.setOutput(true);
    
    // Define a cor do bloco
    this.setColour('#659AA6');
    
    // Define a tooltip para fornecer mais informações sobre a funcionalidade do bloco
    this.setTooltip("Calculate the correlation of the given DataFrame.");
    
    // Define a URL de ajuda (ainda não configurada)
    this.setHelpUrl("");
  }
};
...
pythonGenerator['scatterGraph'] = function(block: any): string {
  ...
  let layout: string = pythonGenerator.valueToCode(block, 'layout', pythonGenerator.ORDER_MEMBER) || '';
  let colour_block = (block.getInputTargetBlock('colour')) ? block.getInputTargetBlock('colour') : '';
  if(block.getInputTargetBlock('colour') && block.getInputTargetBlock('colour')?.type === 'variables_get'){
    var myblocks = Blockly.getMainWorkspace().getAllBlocks()
    for( var i=0; i < myblocks.length; i++){
      if(myblocks[i].type == 'variables_set'){
        var aux = pythonGenerator.nameDB_.getName(myblocks[i].getFieldValue('VAR'), Blockly.VARIABLE_CATEGORY_NAME)
        if(aux === colour){
          colour_block = myblocks[i].getInputTargetBlock('VALUE')
        }
      }
    }
  }
  if(colour_block && (colour_block.type != "lists_create_with" && colour_block.type != "kmeans" && colour_block.type != "select")){
    colour = colour + '.iloc[:, 0]'
  }
  if(block.getInputTargetBlock('x')){
    let block_x_type = block.getInputTargetBlock('x').type
    if(block_x_type != 'lists_create_with' && block_x_type != 'select' && !block.getInputTargetBlock('y')){
      return `sns.scatterplot(x = ${value_x}.index, y = ${value_x}.values, hue = ${colour}).set(**${layout})\n`
    }
  }
  if(block.getInputTargetBlock('y')){
    let block_y_type = block.getInputTargetBlock('y').type
    if(block_y_type != 'lists_create_with' && block_y_type != 'select' && !block.getInputTargetBlock('x')){
      return `sns.scatterplot(x = ${value_y}.index, y = ${value_y}.values, hue = ${colour}).set(**${layout})\n`
    }
  }
  pythonGenerator.definitions_['import_seaborn'] = 'import seaborn as sns';
  return `sns.scatterplot(x = ${value_x}, y = ${value_y}, hue = ${colour}).set(**${layout})\n`
};
